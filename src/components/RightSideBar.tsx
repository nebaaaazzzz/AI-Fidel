...
function RightSideBar() {
  ...
  return (
    <div className="hidden transition-all cxl:hidden csl:flex shrink-0 flex-col max-h-[80vh] items-center gap-1">
      <Icon />
      <div
        className="flex flex-col overflow-y-scroll scrollbar transition-all h-full md:w-42 mt-3 lg:w-56 items-center px-5 rounded-lg py-2"
        style={{
          backgroundImage:
...
function Header({ text }: { text: string }) {
  return (
    <>
      <h1 className="mt-5 mb-1  font-bold text-xl propse text-primary">{text}</h1>
      <Divider />
    </>
  );
}
